...
BLOSUM50=("""kdedeeefdedeecegfcdfa medbgfcfbcidcceececa mhdfffgcbfdbdgfbdca """
          """nbfheebbebaefeacba sccccddcddbeeacea mhdgcdhfbefeeeca """ #03 quote vergeten
...
def align(seq1, seq2, blosum=BLOSUM50, penalty=-50, method="K"): #02 dubbele punt vergeten
    ...
    for i in range(m):
        for j in range(n):
            match  = M[i][j] + blosum[s1[i]][s2[j]] #07 blosum
...
def is_atom(line):
    return line.startswith('ATOM') and line[13:15] == "CA" #06 startswith


def main(args):

    # Mobile
    with open(args[1]) as pdb:#05 pdb ipv pdbfile
        mobile = [ AA321.get(line[17:20],"x") for line in pdb if is_atom(line) ]

    # Mobile
    with open(args[2]) as pdb2: #07 pdb2
        fixed = [ AA321.get(line[17:20],"x") for line in pdb2 if is_atom(line) ]

    aligned1, aligned2, score = align(fixed, mobile) #08returns 3 arguments

    print("Alignment score:", score)

    for i in range(max(len(aligned2), len(aligned1))): #10 range
        if aligned1[i] == aligned2[i]:
            aligned1[i] = RED + aligned1[i] + OFF
            aligned2[i] = RED + aligned2[i] + OFF

    print(OFF)
    for i in range(0, max(len(aligned2), len(aligned1)), 50):
        print("\n{:<10d}{:<10d}{:<10d}{:<10d}{:<10d}".format(i, i + 10, i + 20, i + 30, i + 40, i + 50))
        print(5 * "0123456789")
...
if __name__ == "__main__":
    exitcode = main(sys.argv) #04 sys.argv ipv sys.args
    sys.exit(exitcode)
